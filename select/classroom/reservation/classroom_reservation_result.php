...
if ($UserRole === 'student') {
    $reservationSql = "
    INSERT INTO classroomreservation (ClassroomID, StudentID, ReservationTime)
    VALUES ('$ClassroomID', '$UserID', '$datetimeValue')
    ";
} elseif ($UserRole === 'professor') {
    $reservationSql = "
    INSERT INTO classroomreservation (ClassroomID, ProfessorID, ReservationTime)
    VALUES ('$ClassroomID', '$UserID', '$datetimeValue')
    ";
} else {
    echo "잘못된 사용자 역할입니다.";
...
// 쿼리 실행 및 결과 확인
if (mysqli_query($con, $reservationSql)) {
    echo "<h1>예약 완료</h1>";
    echo "<p>강의실 ID: $ClassroomID</p>";
    echo "<p>예약 시간: $datetimeValue</p>";
    echo "<p>예약이 성공적으로 완료되었습니다.</p>";
} else {
    echo "<h1>예약 실패</h1>";
    echo "<p>강의실 ID: $ClassroomID</p>";
    echo "<p>예약 시간: $datetimeValue</p>";
    echo "<p>예약에 실패하였습니다: " . mysqli_error($con) . "</p>";
}
